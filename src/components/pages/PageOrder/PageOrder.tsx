  ...
  const cartItems: CartItem[] = React.useMemo(() => {
    if (order && products) {
      return order.items.map((item: OrderItem) => {
        ...
        if (!product) {
          console.error(`Product with ID ${item.productId} not found for order item.`);
    ...
    <PaperLayout>
      ...
      <TableContainer>
        <Table aria-label="simple table">
          ...
          <TableBody>
            {statusHistory.map((statusHistoryItem, index) => (
              ...
              <TableRow key={statusHistoryItem.timestamp || `${order.id}-${index}`}>
